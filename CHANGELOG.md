...
### Fixed
- Replaced the self-attested tenant-isolation gate. `/api/beta-readiness` G3 and
  `/api/health` both keyed off `MATIQ_TENANT_ISOLATION_V2="enforced"`, an
  environment variable that asserted nothing about the code: it reported GO
  whether or not routes resolved academies from membership, and reported a false
  HOLD and `degraded` once they did. G3 now measures stored data for records
  that carry no resolvable academy, and health no longer treats a code property
  as a runtime binding.
