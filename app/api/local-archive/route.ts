...
export async function GET(request:NextRequest){
  if(!await selectedAcademy(request,"admin"))return NextResponse.json({error:"forbidden"},{status:403});
  return NextResponse.json({
    ...
    excludedClasses:["minor profiles","guardian records","minor training records","minor competition records","raw media","emails","social links","payment identifiers","authentication tokens"],
...
export async function POST(request:NextRequest){
  ...
  const athleteResult=await env.DB.prepare("SELECT id,alias,birth_year AS birthYear,belt,active,created_at AS createdAt FROM athletes WHERE academy_id=? AND active=1 ORDER BY created_at").bind(academyId).all<Row>();
  const adults=athleteResult.results.filter(row=>Number(row.birthYear)>0&&year-Number(row.birthYear)>=18);
  ...
  if(adultIds.length){
    ...
    for(const [name,sql] of Object.entries(scopedQueries)){
      const result=await env.DB.prepare(sql).bind(academyId,...adultIds).all();
      tables[name]=result.results;
    }
  }
  const academy=await env.DB.prepare("SELECT id,name,plan,created_at AS createdAt FROM academies WHERE id=?").bind(academyId).first<Row>();
  const exportedAt=Date.now();
  const recordCount=adults.length+Object.values(tables).reduce((sum,rows)=>sum+rows.length,0);
  const archive={
    ...
    athletes:adults.map(row=>({id:row.id,alias:row.alias,birthYear:row.birthYear,belt:row.belt,createdAt:row.createdAt})),
    tables
  };
  const payload=JSON.stringify(archive,null,2);
  const checksum=await digest(payload);
  await env.DB.prepare("INSERT INTO audit_events(id,academy_id,actor_email,action,object_type,object_id,outcome,created_at) VALUES(?,?,?,?,?,?,?,?)").bind(crypto.randomUUID(),academyId,identity(request)||"unknown","local_archive_generated","adult_only_history",checksum,"success",exportedAt).run();
