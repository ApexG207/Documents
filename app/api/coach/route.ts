...
export async function POST(request: NextRequest) {
  ...
  try {
        ...
        input: [
          {
            role: "system",
            content:
              "You are a youth jiu-jitsu coaching assistant. Produce one concise, development-first intervention. Do not give medical advice, weight-cut instructions, diagnoses, or guarantees. Use anonymized performance data only. Require adult coach approval.",
          },
          {
            role: "user",
            content: `Build a ${days}-day intervention for the performance focus: ${focus}. Include objective, two drills, one constraint round, one measure, and a parent-facing note.`,
          },
        ],
    ...
    return NextResponse.json({
      brief: data.output_text || fallback,
      mode: "ai",
      review: "coach-required",
    });
  } catch {
    return NextResponse.json({ brief: fallback, mode: "baseline" });
  }
}
